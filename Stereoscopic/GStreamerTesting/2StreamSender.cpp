...
int main(){
    ...
    //VideoWriter out0("appsrc ! v4l2convert ! v4l2h264enc ! fakesink", 0, 30.0, {640, 480}, 1);
    
    VideoWriter out0("appsrc ! v4l2convert ! video/x-raw,format=I420 ! v4l2h264enc ! video/x-h264,level=(string)4.2,profile=baseline ! h264parse ! rtph264pay config-interval=-1 ! udpsink host=172.17.141.179 port=5000", 0, 30.0, {640, 480}, 1);

    //) h264parse ! rtph264pay config-interval=-1 ! udpsink host=172.17.141.179 port=5000",     

    VideoWriter out1("appsrc ! v4l2convert ! video/x-raw,format=I420 ! v4l2h264enc ! video/x-h264,level=(string)4.2,profile=baseline !  h264parse ! rtph264pay config-interval=-1 ! udpsink host=172.17.141.179 port=5001", 0, 30.0, {640, 480}, 1);
    
    //VideoWriter out0("appsrc ! fakesink", 0, 30.0, {640,480}, 1); 
    //VideoWriter out1("appsrc ! fakesink", 0, 30.0, {640,480}, 1);
